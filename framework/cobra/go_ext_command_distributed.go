...
func (c *Command) AddDistributedCronCommand(serviceName string, spec string, cmd *Command, holdTime time.Duration) {
	...
	app := root.GetContainer().MustMake(contract.AppKey).(contract.App)
	distributed := root.GetContainer().MustMake(contract.DistributedKey).(contract.Distributed)
	appID := app.AppId()
	...
	root.Cron.AddFunc(spec, func() {
		...
		// 节点进行选举，返回选举结果
		selectedAppID, err := distributed.Select(serviceName, appID, holdTime)
		if err != nil {
			return
		}
